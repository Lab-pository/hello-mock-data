...
(
    ...
    order_type  varchar(10)  not null check (order_type in ('PENDING', 'SUCCESS', 'FAIL', 'CANCELED')),
    created_at  TIMESTAMP(6) not null
) ENGINE = InnoDB
  DEFAULT CHARSET = utf8mb4;
...
#     truncate payment;
#     update product set quantity = 1000000;
# end;
#
# select
#     count(p.product_id)
...
#         p.end_time > now()
# order by
#     p.start_time limit 20 offset 0;
#
# create index index_product_start_time on product(start_time);
# create index index_product_end_time on product(end_time);
